...
- **Селекторы** — выбор активного прокси в группах прямо из LuCI (через Clash API): клик по карточке — выбрать, ⚡ — замерить задержки, обновление rule/proxy-провайдеров. Главный переключатель **«Весь трафик» (напрямую/через прокси)** + у каждого сервиса свой выбор — можно пустить всё напрямую, а через прокси только выбранное (например, YouTube и Twitch)
...
**Селекторы** — «Весь трафик» (напрямую/прокси), выбор прокси для каждого сервиса, замер задержек, обновление провайдеров. На примере: всё напрямую, а YouTube и Twitch — через прокси:
